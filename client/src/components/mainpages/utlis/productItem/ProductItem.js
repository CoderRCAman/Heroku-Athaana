...
function ProductItem({ product, isAdmin, deleteProduct, handleCheck }) {
    ...
    return (
        <div className="product_card">
            <span className="offer" style={{ display: 'flex', position: 'absolute',left:'15px' }}>{offer ? <span style={{position:'absolute',color:'white',letterSpacing:'1px',backgroundColor:'#577c92',padding:'2px 5px 2px 5px'}}>{offer.toFixed(0)}% off</span> :null}</span>

            {
                isAdmin && <input style={{float:'right'}} type="checkbox" checked={product.checked}
            ...
            <div className="product_box">
                ...
                <div style={{fontSize:'12px'}}>
                    ...
                    {product.pricemk ? <del style={{color:'red'}}>MRP-{product.pricemk}</del>
