...
function Banners() {
    ...
    const createBanner = async e =>{
        ...
        try {
            ...
            if(onEdit){
                const res = await axios.put(`/api/banners/${id}`, {name: banner,images}, {
                    headers: {Authorization: token}
                })
                alert(res.data.msg)
            }else{
                const res = await axios.post('/api/banners', {name: banner,images}, {
                    headers: {Authorization: token}
                })
                alert(res.data.msg)
            }
    ...
    return (
        ...
        <div className="categories">
            <form onSubmit={createBanner}>
                ...
                <input type="text" name="banner" value={banner} required
            ...
            <div className="col">
                {
                    banners.map(banner => (
                        <div className="row" key={banner._id}>
                            <div>
                                <img style={{width:'150px',height:'90px'}} src={banner.images.url} alt=""/>
                            </div>
                            <div>
                            <p style={{marginLeft:"55px",marginBottom:'15px'}}>{banner.name}</p>
                                <button onClick={() => editBanner(banner._id, banner.name)}>Edit</button>
