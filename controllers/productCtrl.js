...
const SortProducts = async (_sort, limit, skip) => {
  ...
  if (_sort === "sort=-sold") {
    products = await Products.find().limit(limit).sort({ sold: -1 ,_id: 1}).skip(skip);
  }
  if (_sort === "sort=-price") {
    products = await Products.find()
      .limit(limit)
  ...
  if (_sort === "sort=price") {
    products = await Products.find().limit(limit).sort({ price: 1 ,_id:1}).skip(skip);
  }
  if (!_sort || _sort === "") {
    products = await Products.find()
      .limit(limit)
...
const productCtrl = {
  getProducts: async (req, res) => {
    try {
      ...
      const count =  category?await Products.countDocuments({category:category}):await Products.countDocuments() ;
      if (!page) {
        page = 1;
      }
      if (!size) {
      ...
      res.status(200).json({ page: page, size: size, products: products,available: (count - page*size) });
    } catch (error) {
      console.log(error);
  ...
  getProductsByCategory : async(req,res) => { 
   ...
   getProduct : async ( req,res) => {
    ...
    createProduct: async (req, res) => {
        try {
            const { product_id, title, price, pricemk,description, content, images, category} = req.body;
            ...
            const newProduct = new Products({
                product_id, title:title.toLowerCase(), price, pricemk,description, content, images, category
            }
            )
    ...
    updateProduct: async (req, res) => {
        try {
            const { title, price, pricemk,description, content, images, category} = req.body;
            if (!images) return res.status(400).json({ msg: "No image upload" })

            await Products.findOneAndUpdate({ _id: req.params.id }, {
                title:title.toLowerCase(), price, pricemk,description, content, images, category
            }
            )
